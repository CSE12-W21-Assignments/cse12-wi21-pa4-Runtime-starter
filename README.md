...
This PA is due on ** **Wednesday, February 3 at 11:59pm** **

Link to FAQ: https://piazza.com/class/kirvel5rj2337n?cid=594
...
## Part 1.1: Big-O Justification

Indicate whether the following assertions are true or false, and give a
justification:

- _n! + n*log n + n_ is _O(n * log n)_
- _n + 5n<sup>2</sup> + 7n<sup>5</sup>_ is _O(n<sup>4</sup>)_
- _5 * log n + n * log n + log(log n)_ is _Ω(n)_
- _n<sup>2</sup> + n/4 + 6_ is _Ө(n<sup>2</sup>)_
- _n/(n<sup>30</sup>) + log32_ is _Ω(1)_
- _n/(n<sup>30</sup>) + log2_ is _O(1)_

If you are justifying the statement is true, give choices of `n0` and `C`. For big-Θ, make sure to justify both big-O and big-Ω.

If you are justifying the statement is false, indicate which term(s) can't work
...
You can also review the lecture notes on runtime found here: https://github.com/ucsd-cse12-w21/ucsd-cse12-w21.github.io/tree/master/lectures/lecture-09
...
## Part 1.2: List Analysis

Consider the two files [`ArrayStringList.java`](https://github.com/ucsd-cse12-w21/ucsd-cse12-w21.github.io/blob/master/lectures/lecture-04/ArrayStringList.java) and
[`LinkedStringList.java`](https://github.com/ucsd-cse12-w21/ucsd-cse12-w21.github.io/blob/master/lectures/lecture-05/LinkedStringList.java) from lecture.   
**Note**: they may or may not have `add` and `prepend` implemented but how would they be implemented?
...
## Part 1.3: Mystery Functions
**Important**: this section can not be completed until you finish Part 2 of the assignment. Please read Part 2 before attempting this secion. 
...
However, in that file, they are called `mysteryA-F`, and they are in a different order, and we don't provide the source of that file. You have two tasks: 1) determining a big-Θ bound for each method labeled 1-6 by analyzing the source above, and 2) determining which mystery method A-F corresponds to the implementations above by measuring against provided (but hidden) implementation.
...
- `measure` should work as follows:
  ...
  code](https://github.com/ucsd-cse12-w21/ucsd-cse12-w21.github.io/tree/master/discussion/Week4))
...
If you would like to run your code from the commandline with the `mysteries.jar` file, use the following commands in the `src` folder:
 
Running on UNIX based (Mac or Linux) systems:
- Compile: `javac -cp mysteries.jar *.java`
- Execute: `java -cp mysteries.jar:. Main`
...
- The `Programming Assignment 4 - questions` assignment in Gradescope, where you will submit the written part of this PA.
  ...
  - The third question is for your matchings for the mystery functions, along with your graphs and justifications. The following are what need to be answered in the subquestions.
      - The BigΘ bounds for each implementation f1-6.
    - A listing that matches each of mysteryA-F to an implementation f1-6 above 
    - Three graphs that justify a few choices above (pdf, png, or jpeg format). These don't need to
...
You may encounter errors if you submit extra files/directories or omit files. You may submit as many times as you like till the deadline. 
